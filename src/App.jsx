...
const APP_VERSION = "2.1.0"; // bump to bust Vercel cache
...
const CHART_DAYS = {"1H":1,"24H":1,"7D":7,"30D":30,"6M":180,"1Y":365,"3Y":1095,"5Y":1825,"ALL":"max"};
async function fetchCGChart(cgId,timeframe) { const days=CHART_DAYS[timeframe]||1; const r=await fetch(`https://api.coingecko.com/api/v3/coins/${cgId}/market_chart?vs_currency=usd&days=${days}${days==="max"?"":"&interval="+(days<=1?"minute":days<=30?"hourly":"daily")}`); if(!r.ok) throw new Error("CG chart"); const d=await r.json(); return d.prices||[]; }
...
async function aiPivot(exitSymbol,priceMap,portfolio){
  ...
  try{
    const r=await fetch("/api/ai-analysis",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({type:"pivot",payload:{exitSymbol,priceMap,portfolio,coins:COINS}}),signal:controller.signal});
...
async function aiDeep(coin,price,change,history){
  ...
  try{
    const r=await fetch("/api/ai-analysis",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({type:"deep",payload:{coin,price,change,history}}),signal:controller.signal});
    clearTimeout(timeout);
    if(!r.ok)throw new Error("Deep AI failed: "+r.status);
    return r.json();
  }catch(e){
    clearTimeout(timeout);
    if(e.name==="AbortError")throw new Error("Analysis timed out. Please try again on a faster connection.");
    throw e;
  }
}
...
export default function SignalPulsePro(){
  ...
  if(localStorage.getItem("sp_is_owner")==="true"){
    setIsOwner(true);
    // Also restore owner user so they go straight to MAIN
    const ownerSess=loadSession();
    if(!ownerSess){
  ...
  if(ownerInput.trim()===OWNER_KEY){
    setIsOwner(true);
    localStorage.setItem("sp_is_owner","true");
    const ownerUser={id:"owner",email:"owner@signalpulse.app",name:"Owner",subscribed:true,provider:"owner"};
  ...
  const openDeep=async(coin)=>{setDeepCoin(coin);setScreen(S.DEEP);if(deepData[coin.symbol])return;setDeepBusy(p=>({...p,[coin.symbol]:true}));try{const result=await aiDeep(coin,prices[coin.cgId]?.usd,prices[coin.cgId]?.usd_24h_change,histories[coin.cgId]);setDeepData(p=>({...p,[coin.symbol]:result}));}catch(e){setDeepData(p=>({...p,[coin.symbol]:{summary:"Analysis unavailable — "+(e.message||"please check your connection and try again."),signal:"HODL",confidence:50,riskLevel:"MEDIUM"}}));}setDeepBusy(p=>({...p,[coin.symbol]:false}));};
...
<div style={{marginTop:16,padding:"12px 14px",background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)",borderRadius:T.r3}}>
  ...
  setScreen(S.MAIN);
        ...
        </Card><Btn variant="ghost" onClick={()=>setScreen(S.LANDING)} style={{marginTop:8,fontSize:13,color:T.t3}}>← Back</Btn>
<div style={{marginTop:16,padding:"12px 14px",background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)",borderRadius:T.r3}}>
  <p style={{fontSize:10,color:T.t3,margin:0,lineHeight:1.8,textAlign:"center"}}>
    <strong style={{color:"rgba(255,255,255,.35)"}}>⚠️ Not Financial Advice.</strong> Signals are AI-generated for informational purposes only. Crypto trading involves risk of loss. SignalPulse Pro is not liable for any trading losses. Always consult a licensed financial advisor before investing.
  </p>
</div>
</div></div>);
  ...
  if(screen==="chart"&&chartCoin){
    ...
    const frames=["24H","7D","30D","6M","1Y","3Y","5Y","ALL"];
        ...
        <div style={{padding:16}}>
          ...
          {adminTab==="overview"&&(<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
              {[
                ...
                ["Active (Paid)",paidUsers.length,T.green2],
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:8}}>
              {[
                ["Monthly MRR (paid only)","$"+monthlyRev.toFixed(2),T.green2],
          ...
          {adminTab==="revenue"&&(<>
            <Card style={{marginBottom:14,background:"linear-gradient(135deg,rgba(16,185,129,.1),rgba(99,102,241,.07))",borderColor:"rgba(16,185,129,.3)",padding:20,textAlign:"center"}}>
              ...
              <p style={{fontSize:13,color:T.t3,margin:0}}>{paidUsers.length} paying subscribers × $19.99/mo{activeUsers.length>paidUsers.length?" ("+( activeUsers.length-paidUsers.length)+" free access)":""}</p>
            </Card>
  ...
  localStorage.removeItem("sp_is_owner");
  disconnectWallet();
